...
const AdminDashboard = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const [usersRes, vendorsRes, productsRes, ordersRes, reviewsRes] = await Promise.all([
        ...
        enquiryAPI.getAllEnquiries().catch(() => ({ data: [] }))
      ])
      ...
      const enquiriesRaw = enquiriesRes?.data?.data || enquiriesRes?.data || []
  ...
  const renderEnquiries = () => (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-gray-900">Enquiries & Messages</h3>
      </div>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div className="flex space-x-3">
                              <select
                                ...
                                onChange={async (e) => {
                                  try {
                                    await enquiryAPI.updateEnquiryStatus(enquiry._id, e.target.value);
                              ...
                              <button
                                onClick={async () => {
                                  if (window.confirm('Delete this message?')) {
                                    try {
                                      await enquiryAPI.deleteEnquiry(enquiry._id);
