  ...
  try {
    ...
    if (role === 'admin') {
      return res.status(403).json({
        status: 'error',
        message: 'Admin accounts cannot be created through regular registration. Please use the admin creation endpoint.'
      });
    }
    ...
    if (process.env.SMTP_HOST && process.env.SMTP_USER) {
      ...
      try {
        ...
        await sendEmail({
          email: user.email,
          subject: 'Verify Your Email - ExpressKart',
          message: `Welcome to ExpressKart! Please verify your email by clicking: ${verificationUrl}`
        });
      } catch (emailError) {
    ...
    res.status(201).json({
      status: 'success',
      message: 'Registration successful! Welcome to ExpressKart.',
      data: {
        user: {
          id: user._id,
          name: user.name,
...
// @access  Public (Token verification handled internally)
router.post('/refresh-token', async (req, res) => {
  try {
    ...
    // Verify token (ignore expiration to allow refreshing expired tokens)
